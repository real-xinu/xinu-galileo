...
status  tftpget(
	...
	byte verbose           /* Verbose level                 */
	)
{
	return tftpget_mb(serverip, filename, &rcv_buf, &rcv_buf_size, 1, verbose);
}
...
status  tftpget_mb(
	uint32 serverip,       /* IP address of server in binary   */
	const char* filename,  /* Name of the file to download		*/
	char** rcv_bufs,       /* Buffer array to hold the file		*/
	...
	)
{
	...
	uint32  curr_used;  /* Amount used in current buffer */

	/* check args */
	
	if(NULL == filename || 0 == serverip || NULL == rcv_bufs || NULL == rcv_buf_sizes || 0 == rcv_buf_count) {
	    kprintf("[TFTP GET] ERROR: Invalid argument\n");
		return SYSERR;
	}
	for(i = 0; i < rcv_buf_count; i++) {
		if(NULL == rcv_bufs[i] || 0 == rcv_buf_sizes[i]) {
			kprintf("[TFTP GET] ERROR: Invalid argument\n");
			return SYSERR;
		}
	}
